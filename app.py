...
def extract_text_from_image(image_path):
    try:
        ...
        if not result.get("IsErroredOnProcessing"):
            ...
            logger.error(f"Error processing image: {result.get('ErrorMessage', 'Unknown error')}")
            return None
...
def identify_medical_condition(text):
    ...
    try:
        ...
        logger.exception(f"Error identifying medical condition: {e}")
...
def get_remedy(condition, remedy_type):
    ...
    try:
        ...
        logger.exception(f"Error getting remedy: {e}")
...
def get_remedy_images(query, num_images=3):
    try:
        with DDGS() as ddgs:
            results = list(ddgs.images(query, max_results=num_images*2))
        unique_images = list(set(result['image'] for result in results))
        return unique_images[:num_images]
    except Exception as e:
        logger.exception(f"Error fetching images for {query}: {e}")
        return []
...
Be thorough in your evaluation."""
    ...
    try:
        full_review = ""
        while len(full_review.split()) < 500:
            completion = client.chat.completions.create(
                model="llama-3.1-70b-versatile",
                messages=[
        ...
        logger.exception(f"Error in reviewing answer: {e}")
...
def process_user_request(query=None, image_path=None):
    try:
        ...
        logger.exception(f"An error occurred in process_user_request: {str(e)}")
        return None
...
def display_results(result):
    if isinstance(result, str):
        ...
        st.warning("DISCLAIMER: The information provided here is for educational purposes only and should not be considered medical advice. Always consult with a qualified healthcare professional before starting any treatment or taking any medication.")
